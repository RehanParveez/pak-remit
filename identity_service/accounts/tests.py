...
class ParentTestCase(APITestCase):
  ...
  def setUpTestData(cls):
    ...
    cls.expired_kyc = KYCProfile.objects.create(
      user=cls.expired_kyc_user, status = 'expired', tier = 'tier1', is_verified=False, verified_at=timezone.now() - timedelta(days=20),
    ...
    cls.pending_kyc = KYCProfile.objects.create(user=cls.pending_kyc_user, status = 'pending', tier = 'tier1',
      is_verified=False)
  ...
  def test_auth_setup(self) -> None:
    ...
    self.assertEqual(self.verified_kyc.status, 'approved')
